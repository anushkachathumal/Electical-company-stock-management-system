...
Public Class frmUnloading_cnt
        ...
        Try
            ...
            If isValidDataset(M01) Then
                If M01.Tables(0).Rows(0)("P01No") >= 1 And M01.Tables(0).Rows(0)("P01No") < 10 Then
                    txtEntry.Text = "UD-00" & M01.Tables(0).Rows(0)("P01No")
                ElseIf M01.Tables(0).Rows(0)("P01No") >= 10 And M01.Tables(0).Rows(0)("P01No") < 100 Then
                    txtEntry.Text = "UD-0" & M01.Tables(0).Rows(0)("P01No")
                Else
                    txtEntry.Text = "UD-" & M01.Tables(0).Rows(0)("P01No")
                End If
            End If
    ...
    Function Search_Records()
        ...
        Try
            Sql = "select *   from T23Unloading_Header inner join M03New_Location on M03Loc_Code=T23To_Loc INNER JOIN T24Unloading_Flutter  ON T23Tr_No=T24Tr_No inner join M11Product_Item on M11Part_No=T24Item_Code where T23Tr_Type='UNLOADING' AND T23Tr_No='" & Trim(txtEntry.Text) & "'  "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            If isValidDataset(M01) Then
                cboTo.Text = Trim(M01.Tables(0).Rows(0)("M03Loc_Name"))
                cboVehicle.Text = Trim(M01.Tables(0).Rows(0)("T23From_Loc"))
                txtDate.Text = Trim(M01.Tables(0).Rows(0)("T23Date"))
                txtRemark.Text = Trim(M01.Tables(0).Rows(0)("T23Remark"))
                ...
                For Each DTRow2 As DataRow In M01.Tables(0).Rows
                    ...
                    newRow("##") = Trim(M01.Tables(0).Rows(i)("T24Usable"))
                    newRow("Item Code") = Trim(M01.Tables(0).Rows(i)("T24Item_Code"))
                    newRow("Item Name") = Trim(M01.Tables(0).Rows(i)("M11Part_Name"))
                    Value = Trim(M01.Tables(0).Rows(i)("T24Cost"))
                    ...
                    Value = Trim(M01.Tables(0).Rows(i)("T24Rate"))
                    _ST = (Value.ToString("0,0.00", System.Globalization.CultureInfo.InvariantCulture))
                    _ST = (String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:0,0.00}", Value))
                    newRow("Retail Price") = _ST

                    newRow("Qty") = Trim(M01.Tables(0).Rows(i)("T24Qty"))
                    ...
                    Value = CDbl(M01.Tables(0).Rows(i)("T24Qty")) * CDbl(M01.Tables(0).Rows(i)("T24Rate"))
                    ...
                    i = i + 1
                Next
                txtCount.Text = UltraGrid2.Rows.Count
    ...
    Function Load_STATUS()
        ...
        Try
            Sql = "select M22Description as [##] from M22Common where  M22Ref='2' "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            With cboStatus
                .DataSource = M01
                .Rows.Band.Columns(0).Width = 91
                '.Rows.Band.Columns(1).Width = 320
    ...
    Private Sub frmUnloading_cnt_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        frmView_Unloading.Close()
    End Sub

    Private Sub frmUnloading_cnt_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Call Load_EntryNo()
    ...
    Function Load_Gride2()
        ...
        c_dataCustomer1 = CustomerDataClass.MakeDataTable_UNLOADING_CNT
    ...
    Private Sub txtQty_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtQty.KeyUp
        ...
        Try
            If e.KeyCode = 13 Then
                ...
                Else
                    MsgBox("Please select the Usable status", MsgBoxStyle.Information, "Information .......")
                    Exit Sub
                End If
    ...
    Function Search_Location() As Boolean
        ...
        Dim i As Integer
        Try
            Sql = "SELECT * FROM M03New_Location WHERE M03Loc_Name='" & Trim(cboTo.Text) & "' AND M03Status='A' "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            If isValidDataset(M01) Then
    ...
    Function Calculate_Net()
        ...
        For Each uRow As UltraGridRow In UltraGrid2.Rows
            If IsNumeric(UltraGrid2.Rows(I).Cells(6).Text) Then
                Value = Value + CDbl(UltraGrid2.Rows(I).Cells(6).Text)
            End If
            I = I + 1
        Next
    ...
    Private Sub UltraGrid1_AfterRowsDeleted(ByVal sender As Object, ByVal e As System.EventArgs)
        Call Calculate_Net()
        txtCount.Text = UltraGrid2.Rows.Count
    End Sub
    ...
    Function Save_Data()
        ...
        Try
            ...
            nvcFieldList1 = "SELECT * FROM T23Unloading_Header WHERE T23Tr_No='" & Trim(txtEntry.Text) & "' "
            MB51 = DBEngin.ExecuteDataset(connection, transaction, nvcFieldList1)
            If isValidDataset(MB51) Then
                MsgBox("This Unloading no alrady exsist", MsgBoxStyle.Information, "Information ......")
                connection.ClearAllPools()
                connection.Close()
                Exit Function

            Else
                nvcFieldList1 = "update P01Parameter set P01No=P01No+ " & 1 & " where P01Code='UD' "
                ExecuteNonQueryText(connection, transaction, nvcFieldList1)

                nvcFieldList1 = "Insert Into T23Unloading_Header(T23Tr_No,T23Tr_Type,T23Date,T23Remark,T23From_Loc,T23To_Loc,T23Status)" & _
                                                                  " values('" & Trim(txtEntry.Text) & "','UNLOADING', '" & _GetDate & "','" & txtRemark.Text & "','" & Trim(cboVehicle.Text) & "','" & _Location & "','A')"
                ExecuteNonQueryText(connection, transaction, nvcFieldList1)

                nvcFieldList1 = "Insert Into tmpMaster_Log(tmpStatus,tmpProcess,tmpTime,tmpUser,tmpCode)" & _
                                                                " values('UNLOADING','SAVE', '" & Now & "','" & strDisname & "','" & txtEntry.Text & "')"
                ...
                I = 0
                For Each uRow As UltraGridRow In UltraGrid2.Rows
                    nvcFieldList1 = "Insert Into T24Unloading_Flutter(T24Tr_No,T24Item_Code,T24Cost,T24Rate,T24Discount,T24Qty,T24Status,T24Usable)" & _
                                                             " values('" & Trim(txtEntry.Text) & "','" & Trim(UltraGrid2.Rows(I).Cells(1).Text) & "', '" & Trim(UltraGrid2.Rows(I).Cells(3).Text) & "','" & Trim(UltraGrid2.Rows(I).Cells(4).Text) & "','0','" & Trim(UltraGrid2.Rows(I).Cells(5).Text) & "','A','" & Trim(UltraGrid2.Rows(I).Cells(0).Text) & "')"
                    ExecuteNonQueryText(connection, transaction, nvcFieldList1)
                    '==================================================================
                    ...
                    nvcFieldList1 = "Insert Into S01Stock_Product_Items(S01Part_No,S01Loc_Code,S01Date,S01Tr_Type,S01Ref_No,S01Qty,S01Status)" & _
                                                          " values('" & Trim(UltraGrid2.Rows(I).Cells(1).Text) & "', '" & Trim(cboVehicle.Text) & "','" & _GetDate & "','UNLOADING','" & Trim(txtEntry.Text) & "','" & -(UltraGrid2.Rows(I).Cells(5).Text) & "','A')"
                    ExecuteNonQueryText(connection, transaction, nvcFieldList1)
                    '==================================================================
                    'UPDATE TO LOCATION 
    ...
    Function EDIT_DATA()
        ...
        Try
            ...
            MB51 = DBEngin.ExecuteDataset(connection, transaction, nvcFieldList1)
            If isValidDataset(MB51) Then
                nvcFieldList1 = "UPDATE T23Unloading_Header SET T23Date='" & _GetDate & "',T23Remark='" & Trim(txtRemark.Text) & "',T23From_Loc='" & _Location & "',T23To_Loc='" & Trim(cboTo.Text) & "',T23Status='A' WHERE T23Tr_No='" & Trim(txtEntry.Text) & "' AND T23Tr_Type='UNLOADING' "
                ExecuteNonQueryText(connection, transaction, nvcFieldList1)

                nvcFieldList1 = "Insert Into tmpMaster_Log(tmpStatus,tmpProcess,tmpTime,tmpUser,tmpCode)" & _
                                                       " values('UNLOADING','EDIT', '" & Now & "','" & _AppUser & "','" & txtEntry.Text & "')"
                ExecuteNonQueryText(connection, transaction, nvcFieldList1)
                ...
                MsgBox("Unloading Note Change successfully", MsgBoxStyle.Information, "Information ..........")
            End If
            ' MsgBox("Record Update successfully", MsgBoxStyle.Information, "Information ..........")
            transaction.Commit()
            connection.ClearAllPools()
    ...
    Function Delete_DATA()
        ...
        Try
            ...
            MB51 = DBEngin.ExecuteDataset(connection, transaction, nvcFieldList1)
            If isValidDataset(MB51) Then
                nvcFieldList1 = "UPDATE T23Unloading_Header SET T23Status='CANCEL' WHERE T23Tr_No='" & Trim(txtEntry.Text) & "' AND T23Tr_Type='LOADING' AND T23STATUS='A' "
                ExecuteNonQueryText(connection, transaction, nvcFieldList1)

                nvcFieldList1 = "Insert Into tmpMaster_Log(tmpStatus,tmpProcess,tmpTime,tmpUser,tmpCode)" & _
                                                       " values('UNLOADING','CANCEL', '" & Now & "','" & _AppUser & "','" & txtEntry.Text & "')"
                ExecuteNonQueryText(connection, transaction, nvcFieldList1)

                nvcFieldList1 = "UPDATE T24Unloading_Flutter SET T24Status='CANCEL' WHERE T24Tr_No='" & Trim(txtEntry.Text) & "' AND T24STATUS='A'"
                ExecuteNonQueryText(connection, transaction, nvcFieldList1)

                nvcFieldList1 = "UPDATE S01Stock_Product_Items SET S01Status='CANCEL' WHERE S01Ref_No='" & Trim(txtEntry.Text) & "' AND S01STATUS='A' "
                ExecuteNonQueryText(connection, transaction, nvcFieldList1)

                MsgBox("Unloading Note Canceled successfully", MsgBoxStyle.Information, "Information ..........")
            End If
            ' MsgBox("Record Update successfully", MsgBoxStyle.Information, "Information ..........")
            transaction.Commit()
            connection.ClearAllPools()
    ...
    Private Sub cboVehicle_KeyUp1(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cboVehicle.KeyUp
        If e.KeyCode = 13 Then
