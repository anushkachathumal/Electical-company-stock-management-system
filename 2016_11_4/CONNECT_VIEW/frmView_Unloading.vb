...
Public Class frmView_Unloading
    ...
    Function Load_Grid()
        ...
        Dim M01 As DataSet
        Try
            Sql = "select  ROW_NUMBER() OVER (ORDER BY  T23Tr_No  ) as  ##, T23Tr_No as [Unloading No],MAX(T23Date) as [Date],max(T23From_Loc) as [Vehicle No],MAX(M03Loc_Name) as [To Location],CAST(SUM(T24Qty*T24Cost) AS DECIMAL(16,2)) as [Total Amount]   from T23Unloading_Header inner join M03New_Location on M03Loc_Code=T23To_Loc INNER JOIN T24Unloading_Flutter  ON T23Tr_No=T24Tr_No  where T23Tr_Type='UNLOADING' AND T23Status='A' GROUP BY T23Tr_No ORDER BY max(T23ID)  "
    ...
    Function Load_Grid_Date(ByVal strCode As String)
        ...
        Try
            ...
            Sql = "select  ROW_NUMBER() OVER (ORDER BY  T23Tr_No  ) as  ##, T23Tr_No as [Unloading No],MAX(T23Date) as [Date],max(T23From_Loc) as [Vehicle No],MAX(M03Loc_Name) as [To Location],CAST(SUM(T24Qty*T24Cost) AS DECIMAL(16,2)) as [Total Amount]   from T23Unloading_Header inner join M03New_Location on M03Loc_Code=T23To_Loc INNER JOIN T24Unloading_Flutter  ON T23Tr_No=T24Tr_No  where T23Tr_Type='UNLOADING' AND T23Status='" & strCode & "' AND T23Date BETWEEN '" & txtDate5.Text & "' AND '" & txtDate6.Text & "' GROUP BY T23Tr_No ORDER BY max(T23ID)  "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            UltraGrid1.DataSource = M01
            UltraGrid1.Rows.Band.Columns(0).Width = 30
            UltraGrid1.Rows.Band.Columns(1).Width = 80
    ...
    Function Load_Grid_Vehicle(ByVal strCode As String)
        ...
        Try
            ...
            Sql = "select  ROW_NUMBER() OVER (ORDER BY  T23Tr_No  ) as  ##, T23Tr_No as [Unloading No],MAX(T23Date) as [Date],max(T23From_Loc) as [Vehicle No],MAX(M03Loc_Name) as [To Location],CAST(SUM(T24Qty*T24Cost) AS DECIMAL(16,2)) as [Total Amount]   from T23Unloading_Header inner join M03New_Location on M03Loc_Code=T23To_Loc INNER JOIN T24Unloading_Flutter  ON T23Tr_No=T24Tr_No  where T23Tr_Type='UNLOADING' AND T23Status='" & strCode & "' AND T23Date BETWEEN '" & txtA1.Text & "' AND '" & txtA2.Text & "' AND T23From_Loc='" & Trim(cboVehicle.Text) & "' GROUP BY T23Tr_No ORDER BY max(T23ID)  "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            UltraGrid1.DataSource = M01
            UltraGrid1.Rows.Band.Columns(0).Width = 30
            UltraGrid1.Rows.Band.Columns(1).Width = 80
    ...
    Function Load_Grid_Item(ByVal strCode As String)
        ...
        Dim M01 As DataSet
        Try
            Sql = "select  ROW_NUMBER() OVER (ORDER BY  T23Tr_No  ) as  ##, T23Tr_No as [Unloading No],T23Date as [Date],T23From_Loc as [Vehicle No],M03Loc_Name as [To Location],T24Item_Code as [Item Code],M11Part_Name as [Item Name], T24Qty as [Qty]   from T23Unloading_Header inner join M03New_Location on M03Loc_Code=T23To_Loc INNER JOIN T24Unloading_Flutter  ON T23Tr_No=T24Tr_No inner join M11Product_Item on M11Part_No=T24Item_Code where T23Tr_Type='UNLOADING' AND T23Status='" & strCode & "' and T23Date between '" & txtC1.Text & "' and '" & txtC2.Text & "'  and T24Item_Code='" & Trim(cboItem.Text) & "'  ORDER BY T23ID  "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            UltraGrid1.DataSource = M01
            UltraGrid1.Rows.Band.Columns(0).Width = 30
            UltraGrid1.Rows.Band.Columns(1).Width = 80
    ...
    Private Sub frmView_Unloading_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Call Load_Grid()
    End Sub
    ...
    Private Sub UltraGrid1_DoubleClickRow(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.DoubleClickRowEventArgs) Handles UltraGrid1.DoubleClickRow
        ...
        frmUnloading_cnt.txtEntry.Text = Trim(UltraGrid1.Rows(_Row).Cells(1).Text)
        Call frmUnloading_cnt.Search_Records()
